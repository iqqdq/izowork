...
class DealCalendarViewModel with ChangeNotifier {
          ...
          if (_deals.isNotEmpty) {
            _deals.forEach((element) {
              _eventDateTimes.add(DateTime(
                element.finishAt.year,
                element.finishAt.month,
                element.finishAt.day,
              ));
            });
          }
          notifyListeners();
  ...
  int getDealCount(DateTime dateTime) {
    ...
    _deals.forEach((element) {
      if (dateTime.year == element.finishAt.year &&
          dateTime.month == element.finishAt.month &&
          dateTime.day == element.finishAt.day) {
        count++;
      }
    });
