...
class ObjectPageViewModel with ChangeNotifier {
  ...
  ObjectPageViewModel(this.objectId) {
    checkIsDirector().whenComplete(() => getObjectById(objectId));
  }

  Future checkIsDirector() async =>
      _isDirector = (await GetIt.I<LocalStorageRepositoryInterface>().getUser())
                  ?.state
                  .toLowerCase() ==
              'director'
          ? true
  ...
  void showObjectCreateSheet(BuildContext context) {
    Navigator.push(
        context,
        MaterialPageRoute(
